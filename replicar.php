...
if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
	...
	if (is_dir($diretorio)) {
		...
		// Aqui você pode tratar o caso de não estar em Jundiaí
	}
} else {
	$linux = 1;
...
if (strpos($dir, "com_virtuemart") == true) {
	$tipo = "virtuemart";
	$retirar = substr($dir.'com_virtuemart', 0, strpos($dir, 'com_virtuemart'));
	$dir = str_replace($retirar,"",$dir);
}
if (strpos($dir, "virtuemart_front") !== false) {
    ...
    // Pega a posição onde 'virtuemart_front' começa
    $pos = strpos($dir, 'virtuemart_front');

    // Pega tudo que vem DEPOIS de 'virtuemart_front'  → /views/invoice/tmpl/
    $resto = substr($dir, $pos + strlen('virtuemart_front'));

    // Monta o novo dir substituindo 'virtuemart_front' por 'virtuemart'
...
if ($sacos_teste == "S" || $sacos_teste == "s"){
	if ($dir_meio != "Sacos Bay Plastic" && file_exists($mydir."/".$nome_arquivo)){
		replicar::copiarArquivo($mydir."/".$nome_arquivo,$plat."/Sacos Bay Plastic/".$ano."/".$dir."/",$nome_arquivo);
	} else {
		if (strpos($nome_arquivo, "pasta") !== false || strpos($nome_arquivo, "Pasta") !== false){
			$nome_pasta = str_replace(array("pasta=","Pasta="),"",$nome_arquivo);
...
if ($comary == "S" || $comary == "s"){
	if ($dir_meio != "Clube Comary" && file_exists($mydir."/".$nome_arquivo)){
		replicar::copiarArquivo($mydir."/".$nome_arquivo,$plat."/Clube Comary/".$ano."/".$dir."/",$nome_arquivo);
	} else {
		if (strpos($nome_arquivo, "pasta") !== false || strpos($nome_arquivo, "Pasta") !== false){
			$nome_pasta = str_replace(array("pasta=","Pasta="),"",$nome_arquivo);
			$src = $mydir."/".$nome_pasta;
...
if ($ifen == "S" || $ifen == "s"){
	if ($dir_meio != "IFEN" && file_exists($mydir."/".$nome_arquivo)){
		replicar::copiarArquivo($mydir."/".$nome_arquivo,$plat."/IFEN/".$ano."/".$dir."/",$nome_arquivo);
	} else {
		if (strpos($nome_arquivo, "pasta") !== false || strpos($nome_arquivo, "Pasta") !== false){
			$nome_pasta = str_replace(array("pasta=","Pasta="),"",$nome_arquivo);
			$src = $mydir."/".$nome_pasta;
...
if ($ceerj == "S" || $ceerj == "s"){
	if ($dir_meio != "CEERJ" && file_exists($mydir."/".$nome_arquivo)){
		replicar::copiarArquivo($mydir."/".$nome_arquivo,$plat."/CEERJ/".$ano."/".$dir."/",$nome_arquivo);
	} else {
		if (strpos($nome_arquivo, "pasta") !== false || strpos($nome_arquivo, "Pasta") !== false){
			$nome_pasta = str_replace(array("pasta=","Pasta="),"",$nome_arquivo);
			$src = $mydir."/".$nome_pasta;
...
if ($aequor == "S" || $aequor == "s"){
	if ($dir_meio != "Aequor" && file_exists($mydir."/".$nome_arquivo)){
		replicar::copiarArquivo($mydir."/".$nome_arquivo,$plat."/Aequor/".$ano."/".$dir."/",$nome_arquivo);
	} else {
		if (strpos($nome_arquivo, "pasta") !== false || strpos($nome_arquivo, "Pasta") !== false){
			$nome_pasta = str_replace(array("pasta=","Pasta="),"",$nome_arquivo);
			$src = $mydir."/".$nome_pasta;
...
if ($sistema == "S" || $sistema == "s"){	
	if (file_exists($mydir."/".$nome_arquivo)){
		...
		//Daqui pra baixo só tem $plat de sistema
		if ($win == 1){
			$plat = "D:";
		} else if ($linux == 1){
			$plat = "/media/daniel_alves/Novo volume";
